...
- **`LinkBuilder::build()` signature deviation.** Implemented as `build(string $book, string $passage, string $language)` rather than `build(Reference $ref, string $language)`. The passage segment produced by `linkify()` can be a composite expression (e.g. `4:13;6:1-6`) that does not round-trip through a single `Reference`, so the builder takes the raw book abbreviation and passage string directly.
- **`parser.php` fixture not created.** Task 15 listed a `parser.php` fixture file; in practice `ReferenceParserTest` uses inline test methods because the parser's failure modes are structurally heterogeneous (distinct exception factories with different context shapes), which does not benefit from a flat `{input, expected}` table. Human-readable / canonical / linkify fixtures are still data-driven as planned.
